/* program to define a structure and pointer to it then
...
main()
{
	struct student std1,std2,*stdp;
	clrscr();
	/* assigning the structure address to a structure pointer */
	stdp = &std1;
	...
	printf("Student 1:\nrollno: %d\nname: %s\naverage: %f",stdp->rollno,stdp->name,stdp->average);
